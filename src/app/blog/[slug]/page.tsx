...
export default function BlogPost({ params }: { params: { slug: string } }) {
  ...
  const renderContent = (content: string) => {
    return content.split('\n\n').map((paragraph, idx) => {
      if (paragraph.trim().startsWith('### ')) {
        return (
          <h3 key={idx} className="text-xl md:text-2xl font-bold text-[#1A1F1F] dark:text-white mt-8 mb-3 tracking-tight">
            {paragraph.replace('### ', '')}
          </h3>
        );
      }
      ...
      let formatted = paragraph
        .replace(/\*\*(.*?)\*\*/g, '<strong class="text-[#1A1F1F] dark:text-white font-semibold">$1</strong>')
        .replace(/\*(.*?)\*/g, '<em class="text-[#476550] dark:text-[#2DD4BF] italic">$1</em>');

      if (paragraph.trim().startsWith('- ')) {
        const listItems = paragraph.split('\n').map(item => item.replace('- ', ''));
        return (
          <ul key={idx} className="list-disc pl-6 mb-6 space-y-2 text-[#596060] dark:text-[#CBD5E1] text-base leading-relaxed">
            {listItems.map((li, i) => (
              <li 
                key={i} 
      ...
      return (
        <p 
          key={idx} 
          className="text-[#596060] dark:text-[#CBD5E1] leading-relaxed mb-6 text-base md:text-lg" 
  ...
  return (
    <article className="max-w-4xl mx-auto py-8 px-2 md:px-4">
      ...
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-[#596060] dark:text-[#94A3B8] mb-6 overflow-x-auto">
        <Link href="/" className="hover:text-[#476550] dark:hover:text-[#2DD4BF] transition-colors">Home</Link>
        <ChevronRight className="w-3.5 h-3.5" />
        <Link href="/blog" className="hover:text-[#476550] dark:hover:text-[#2DD4BF] transition-colors">Blog</Link>
        <ChevronRight className="w-3.5 h-3.5" />
        <span className="text-[#476550] dark:text-[#2DD4BF] font-semibold truncate max-w-[220px] md:max-w-md">{post.title}</span>
      </nav>
      ...
      <header className="mb-8">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold text-[#1A1F1F] dark:text-white mb-6 leading-tight tracking-tight">
          {post.title}
        </h1>

        <p className="text-lg md:text-xl text-[#596060] dark:text-[#CBD5E1] leading-relaxed mb-6 font-light">
          {post.excerpt}
        </p>
        ...
        <div className="bg-[#FAF9F6] dark:bg-[#141B20] p-4 rounded-2xl border border-[#D8E2DA] dark:border-[#232D36] shadow-soft flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#596060] dark:text-[#94A3B8]">{post.author.role}</p>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <h4 className="text-lg font-bold text-[#1A1F1F] dark:text-white">{post.author.name}</h4>
              <span className="text-xs text-[#476550] dark:text-[#2DD4BF] font-semibold">({post.author.experience})</span>
            </div>
            <p className="text-xs text-[#596060] dark:text-[#94A3B8] mb-2">{post.author.role}</p>
            <p className="text-sm text-[#596060] dark:text-[#CBD5E1] leading-relaxed">{post.author.bio}</p>
          </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {otherPosts.map((other) => (
            ...
            >
              ...
              <p className="text-xs text-[#596060] dark:text-[#94A3B8] line-clamp-2">{other.excerpt}</p>
            </Link>
          ))}
        </div>
      </section>
